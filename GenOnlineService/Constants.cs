...
namespace GenOnlineService
{
    public static class Constants
    {
        ...
        public static readonly FrozenDictionary<Int16, RoomData> Rooms;

        static Constants()
        {
            string strFileData = System.IO.File.ReadAllText(Path.Combine("data", "rooms.json"));
            Rooms = JsonSerializer.Deserialize<List<RoomData>>(strFileData).ToFrozenDictionary(x => x.id);
        }
    }
    ...
    public class RoomData
    {
        ...
        public string name
        {
            ...
            {
                m_name = value;
                SearchableName = value.Replace(" ", string.Empty).ToLowerInvariant();
            }
        }
        public ERoomFlags flags { get; init; } = ERoomFlags.ROOM_FLAGS_DEFAULT;

        [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
        public string? exclusionKey { get; init; } = null;
